...
export function normalizeOptions(options?: BlueMatterOptions): NormalizedOptions {
  ...
  const rawDelimiters = arrayify(opts.delims ?? opts.delimiters ?? '---');
  const delimiters = [...rawDelimiters];
  if (delimiters.length === 1) {
    delimiters.push(delimiters[0] as string);
  }

  const language = (opts.language ?? opts.lang ?? 'yaml').toLowerCase();
  const engines = { ...defaultEngines, ...opts.parsers, ...opts.engines };
